...
void figure20_peakmatch_overlay(const char* pythiaFile = "output.root",
  ...
  for (int c : cuts) {
    ...
    std::cout << "cut " << c
              ...
              << " , peak-match scale K = " << sharedScale[c] << "\n";
  }

  TString tag(outputTag);
  if (tag.Length() == 0) tag = "public";

  auto cCombinedCounts = new TCanvas(Form("cFigure20Peakmatch_%s", tag.Data()),
                                     Form("Figure 20 peak-matched overlay %s", tag.Data()),
                                     1400, 900);
  ...
  TString rootName = Form("figure20_peakmatch_overlay_%s.root", tag.Data());
  TString pdfName  = Form("figure20_peakmatch_overlay_%s.pdf", tag.Data());
  TString pngName  = Form("figure20_peakmatch_overlay_%s.png", tag.Data());
  TString txtName  = Form("figure20_peakmatch_overlay_chi2_%s.txt", tag.Data());

  std::ofstream foutTxt(txtName.Data());
  foutTxt << "Figure 20-style OS-peak-matched overlay\n";
  foutTxt << "PYTHIA file: " << pythiaFile << "\n";
  foutTxt << "Theory file: " << theoryFile << "\n\n";
  foutTxt << "Peak-matched scales per cut\n";
  for (int c : cuts) {
    foutTxt << "cut " << c << "% : scale = " << sharedScale[c] << "\n";
  }
  foutTxt << "\n";
...
void resultsfinal_smoothTheory_sharedPeak_100M_chi2() {
  figure20_peakmatch_overlay("output.root",
                             "data/theory/epemCrossSection_z0p70.dat",
                             "public");
}
