...
This repository contains the analysis code, documentation, thesis, and presentation material from my BSc Theoretical Physics thesis project at the UCD School of Physics:
...
The repository is simulation-level: it does not contain detector-corrected LEP data or a fitted TMD extraction.
...
The repository keeps the final PYTHIA analysis, selected plotting scripts, compact theory curves, thesis material, and presentation provenance. Large generated ROOT files, temporary logs, binaries, and exploratory prototypes are excluded.
...
The analysis studies whether a charged-pion-pair observable in opposite jets produces a stable opposite-sign versus same-sign separation in simulation, and how that separation changes as pion momentum-fraction cuts are tightened.
...
Builds are handled by the root-level `Makefile`. It wraps the ROOT/PYTHIA/FastJet compile and run commands used for the thesis analysis; no CMake project is required.
...
Then edit `local.mk` with the correct config-command paths. The file `local.mk` is ignored by git, so each user can keep machine-specific ROOT/PYTHIA/FastJet paths outside the tracked repository.

## Quick Start

From the repository root, build the analysis and run a short PYTHIA sample:
...
The Makefile is the recommended interface because it keeps ROOT, PYTHIA, and FastJet configuration in one place and sets runtime library paths only for the analysis command. The compiled executable can also be run directly:

```bash
./tmd-pythia [events] [seed] [output-root-file] [progress-file]
```

The quick-start command is modest by design. Thesis-scale runs used much larger samples, up to 100M events, and produced large ROOT files that are excluded from GitHub.
...
The PYTHIA study found a clear qualitative trend: tightening pion momentum-fraction cuts increased the separation between opposite-sign and same-sign selected pion-pair qT spectra.

Independent TMD-based theory curves were used as a comparison to the PYTHIA-level observable. The comparison is a physics guide and does not constitute a full TMD extraction or detector-corrected LEP measurement.
